...
- **来源**：A2 交回时指出——它的改动把该文件从 492 推到 500，正卡在「复杂文件 500 行」上限，
  下一次改同一文件必然顶破
- **状态**：DONE f3a0a4d（15 例拆成 12+3，一例不减；验收逐条追了被删的 6 行注释，全部随代码搬到新文件，无一被砍。harness 里还写了「不含 vi.mock，因为它按文件 hoist，塞进共享模块会破坏语义」）
...
- **模型**：opus（跨进程契约）
- **铁律的三道保证**（交回原文）：① 后端判定函数签名就是 `kind → verdict`，物理上够不到 message，
  kind 是闭合 union、对端插不进来；② 前端拿到裁决**第一步就 return**，下面的消息规则一条都不跑——
  「哪些 kind 的 message 是对端写的」这个问题从此不需要回答（那张手工表已删）；③ 拿不到/形状不对的
  裁决一律退到「可重试」，永久结论只可能由后端显式给出。
...
- **连锁（E1 留）**：`CLAUDE.md:128` 现在写着「openai-compat 今天只有 CLI 在用，web 侧传输尚未接入」——**本卡完成后要回来把这句改掉**，否则指令文件当场过时。
- **选定的替代约束**：**显式许可清单 + 结构判据**（不是回环限制——回环会砍掉一半真实用法，
  第三方托管的兼容端点都在公网 https 上）。两条一起才成立：① 信封里**没有 origin 字段**，
  origin 由宿主查配置得出，没登记就 fail closed，且无默认接入点分支；② 登记值须过结构判据
  （https 或指向回环的 http、不许内嵌用户名密码、不许带 query/fragment、长度有顶、归一化去尾斜杠），
  **写入时判一次、每次上行前再判一次**（配置文件用户可手改，不是可信输入）。
- **挡不住的（交回如实列出）**：用户**故意**登记恶意 https 端点（那是他对自己那把 Key 的处置权）；
  不做 TLS 证书固定；不阻止把回环例外指向本机其他服务；DNS 解析到哪不归它管。
- **副作用**：`commandArgs.ts` 原 296 行、加 3 条命令必然顶破，按规则本次就拆（model 域十条入参
  搬到 `model/commandArgs.ts`，根表 extends，剩 278 行）；`model/providerRoute.ts` 现**正好 300 行**，
  没余量了。另：`pnpm dev` 的 Vite 中继没有 openai-compat 路由，会 fail closed 拒绝（中继在
  `scripts/`，不在本卡改动面，且凭据宿主本来就只有 server 一态）。
- **连锁已兑现**：`CLAUDE.md:131` 那句「web 侧传输尚未接入」已改成登记式 origin 的描述（`13f3687`）。
- **状态**：DONE 51aedcf（验收变异：放行明文 http 远端 → 3 条红，还原后 160 绿）
...
- **模型**：sonnet
- **状态**：DONE 333870c（六处逐条附了验证命令；验收独立复核了 `runToolLoop.ts:151-166` 的兜底分支）
...
- **抽象决策**：`ToolRepetitionTracker` **只负责数**，不负责「哪几轮该数」也不负责「数到了做什么」
  ——这两件事留在各自调用侧，正是「插件 / 非插件」两种装配形态的分界：主 run 只在纯工具轮累计、
  其余轮 `reset()` 并在命中时终止 run；子 run 从不 `reset()`、也从不改控制流。差异做成 `reset()` 由
  调用方触发，**而不是往判据里加 mode 开关**。
- **信息落点**：`ChildAgentResult.summary`——父 agent 经 `join_agent` / `observe_agent` 拿到的
  `data.children[].summary` 就是这份文本。只写进异常串**不够**：末轮是强制合成轮，撞上限的子 run
  通常从那一轮正常返回、状态 `done`，压根走不到 throw。
- **状态**：DONE 86c0dd9（验收变异：把共用阈值抬到 999 → 主 run 12/20 红、子 run 8/12 红，两侧都用它）
...
- **规则 2 冲突的处理**：负责人说「都能改 atom」，但外部插件代码在磁盘上（`.webAgent/plugins/`），
  `check:state` 规则 2 **扫不到它**——交出 einfach 的 `Store` 等于开一条门禁永远看不见的写入路径，
  绕过事务日志的写入会让 undo 只回滚一部分状态，且只在崩溃恢复时才浮出来。本卡因此只放开
  **拦截与上下文改写**，`PluginHookContext` 里没有 store/root/history（端到端测试断言了拿不到）。
...
- **它自己翻的车（如实报告，值得记）**：第一版把「分组成员」判成「有 `package.json` 的目录」，
  而 **`apps/web` 根本没有 `package.json`**（Vite 的 app root，包身份挂在仓库根）。结果 165 个 UI 文件、
  包括规则 5 的头号案发现场 `UndoBar.tsx` 一声不响掉出扫描面，**`pnpm check:state` 依旧全绿**——
  正是「收窄扫描面让门禁变松而不报错」这个失败模式。它逐文件 diff 才发现，改成「有 `src/` 的目录」
  并补了一道闸：**白名单外出现非测试源文件就抛错**，下次漏根不必靠人工数文件。
- **计数**：1466 → 830，差额 636 全是非源码（610 个 `dist/**` 的 `.d.ts` + 18 个 `tsup.config.ts` +
  8 个手写环境声明），真源码一个没少；规则 3/4 的分类计数收窄前后**逐字相同**，没有规则悄悄改判。
- **防回归测试的关键设计**：测试**独立重算**一遍「哪些 `<成员>/src` 存在」再与 `sourceRoots()` 逐项比，
  **不复用被测模块的枚举**——否则它漏了谁，测试也跟着漏。
- **状态**：DONE 3e701d0（验收变异：拿掉 apps 分组 → 5 条红；另端到端造违规探针确认规则 5 仍咬得住 apps/web）
