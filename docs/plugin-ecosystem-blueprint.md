...
> **交付状态（截至 P11，2026-08-13；宿主口径随 T1 更新）**：第 3 节全部交付——目录约定（3.1）、
> 加载协议（3.2，manifest 解析 + 动态导入 + branded 导出校验）、错误隔离（3.3）与宿主接线（3.4）；
> 第 4 节工具闸门已交付（`pluginToolGate.ts` 默认不注册 + 面板逐工具勾选、记录按用户存）；
> 第 5 节设置面板与熔断（连续 3 次失败自动停用）已交付并随 `main.tsx` 启动装配；20 分钟上手验收见
> [`plugin-quickstart.md`](plugin-quickstart.md)。**未交付**：第 6 节 npm 分发仍阻塞于 G4
> （core 公开面收敛），且已被「不发布、仅本地跑」的裁决按下；第 7 节 `timeline.persist`
> 阻塞于 R5 RFC 未批准。
>
> **3.4 的宿主分类已被后续事实推翻，见该节的更正块。**
...
| loop hook | 7 个槽全部接线，失败隔离并写 trace；外部插件经 `PluginRunApi.hook` 拿到**同一批** 7 个槽（4.3 的信任裁决） | hook 没有稳定的插件身份维度；无法按插件单独停用 |
...
> **⚠️ 更正（T1 之后）：下表的分类错了一处，而且错在判据上。** 它把"支持不支持"绑在
> 「是不是 Tauri」上，于是浏览器整类被排除。真正的判据是**有没有登记宿主命令桥**：插件加载走
> `readWorkspaceFile`，H 线把它的判据换成 `hasHostBridge()` 之后，配了本机 Node 后端的浏览器
> （`pnpm serve`）就有 workspace 文件系统了。桌面壳已随 T1 删除，那一行的实现
> （`apps/web/src/plugins/workspacePluginProvider.ts`，原名 `desktopProvider.ts`）
> **没有删，因为它从来不依赖桌面**——只有文件名骗了人。今天的真实分类是下表加粗的部分。

| 宿主 | 加载机制 | 诚实的风险描述 | 现状 |
| --- | --- | --- | --- |
| **浏览器 + 本机 Node 后端**（原表写的是"桌面（Tauri）"） | 经宿主命令桥读文件 → 前端 `import()` blob URL 求值 | **这是在渲染进程里执行第三方代码，与页面同权**：可触达宿主命令桥、DOM、`fetch`。manifest 的 `capabilities` 是**申报，不是沙箱**，不得在 UI 上暗示它有强制力。当前页面与 `apps/server` 都不注入 CSP；将来加 CSP 必须同时放行 `script-src blob:`，否则这条路径会被静默挡死。 | 支持 |
...
| **纯静态产物**（原表写的是"浏览器预览"） | 没有宿主命令桥，扫不到任何 workspace 文件 | **不要**为它造 dev server 读盘端点——那等于给浏览器开一条任意文件读取通道。 | **不支持**，设置页明示"当前宿主不支持用户插件" |
...
### 4.3 默认姿态 —— 已拍板：目录存在即信任

~~默认关（每个插件都要确认）还是默认开（目录存在即信任），**本蓝图不替用户拍板**，列入第 9 节。~~

**负责人 2026-08-20 拍板：装插件 = 完全信任，不做逐插件确认门。** 同一次裁决把 7 个 loop hook
槽整批开给外部插件（原话「给，同等权利」），其中 `beforeToolCall` 能否决任何一次工具调用（含
shell 命令），`transformContext` / `prepareRequest` 能改模型这一轮看到的上下文。理由是 4.2 已经
写明的那件事：插件入口是宿主 `import()` 求值的**同权代码**，不经过任何 hook 也能直接 fetch、
触达宿主命令桥，再补一道只挡 hook 的确认门是安全剧场。代价必须说在明处，因此：
...
| 宿主 | 单一 CLI | 浏览器（配本机后端）+ CLI，一次声明多处生效；纯静态产物明确不支持 |
...
| 快捷键扩展 | 属于宿主输入层，各宿主键位语义不一致（写这条时是三个宿主，今天是两个），先做只会固化一个错误抽象。 |
...
1. ~~**默认信任姿态**：扫描到的插件默认停用（逐个确认）／默认启用但首次加载前确认／目录存在即
   信任。本蓝图倾向"默认停用"，但不替你决定。~~ **已拍板（负责人 2026-08-20）：目录存在即信任，
   装插件 = 完全信任，不做确认门。** 落地与代价见 4.3。
2. **首期支持宿主**：~~仅桌面／桌面 + CLI（本蓝图倾向）／三宿主都要~~。**这条已被事实回答**：
   桌面壳删除、浏览器接上本机 Node 后端之后，剩下的两个宿主都支持，纯静态那一态因为没有桥
   而天然不支持。本蓝图当年"不要为浏览器另造读盘通道"的反对意见仍然成立——今天走的不是
   dev server 读盘端点，而是同一条 workspace confinement 内的宿主命令桥。
