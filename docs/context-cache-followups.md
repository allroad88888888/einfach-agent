...
**增量样本也不能验收**：SQLite 一致性副本中的最后事件是 **2026-08-04 09:23:03**。新增的两个
`llm.context_snapshot` 都只有 1,681 `estimated_tokens`，没有压缩或投影复用；一个关联的
`llm.chat` 成功、另一个失败。更重要的是，它们早于当前 trace 契约提交 `8148390`
（成功请求按 `(run_id, llm_turn)` 关联，10:05）和 `141ac61`（`dynamic_controls_count`，10:07），
因此 span 中没有 `llm_turn`、快照中也没有 `dynamic_controls_count`，不能用于 F1、F2 或 F6。

**下一次采样前置条件**：重启（或确认完整热重载）包含上述提交的桌面构建；当前 `pnpm tauri dev`
进程启动于 09:16，早于它们。之后完成至少一个越过 200,000 token 软上限的真实长会话，再按本节
和 `context-cache-cost.md` 的 SQL 统计。
...
**做法**：F1 的验证 SQL ③ 现按 `dynamic_controls_count` 分组。只在
`dynamic_controls_count = 0` 的组里要求该原因归零；大于 0 的组单独观察其数量和尾巴体积。若空尾巴
组仍非零，说明仍有常驻内容被错误地放在历史之后；再用 `dynamic_control_changed` 的伴随情况定位。
