...
`purpose=image`、`ms://` 等 Kimi 细节泄漏到 Core 或
Tauri 命令中。
...
- Core 已兼容旧 `string` 与 provider-neutral 结构化用户内容；队列、checkpoint、恢复、搜索、压缩、
  缓存指纹与标题派生均使用统一投影。
- 桌面侧已从单一 chat proxy 收敛为固定 `(provider, scope, method, path)` 的通用 JSON/multipart 传输；
  adapter 仍独占上传字段、远端引用及响应解析。
...
关键边界：**上传是 Kimi adapter 的行为；Tauri 只是一条安全的通用 provider transport。**
...
1. Core 与 Tauri 的职责表无 Kimi 协议泄漏。
