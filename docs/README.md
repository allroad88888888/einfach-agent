...
| [插件上手：20 分钟写一个外部插件](plugin-quickstart.md) | 从零写 `plugin.json` + 单文件 ESM 入口、放进 `.webAgent/plugins/`、用 CLI 验证生效；当前边界（模型可见工具默认不可见、`entry.react` 未接线、纯静态产物不支持） |
| [用户配置目录](config-directory-override.md) | 默认 `~/.webAgent/config.json`、旧配置安全复制、`WEB_AGENT_CONFIG_DIR` 多实例隔离与密钥边界 |
| [卡顿诊断日志](performance-diagnostics.md) | 长对话、plan 更新、大文件写入的分段耗时；只有前端一半，宿主内部目前没有分段日志 |
...
| [Kimi Provider 接入蓝图](kimi-provider-integration-blueprint.md) | Kimi 接入的本地树形 Issue、依赖批次、逐项执行模型与开放门禁；蓝图保留旧 API ID 作为历史记录，当前实现使用 `kimi-k3`，图片输入门禁关闭 |
| [启动模型密钥门禁 Issue](startup-model-credential-gate-blueprint.md) | 已完成：渲染主工作区前按恢复会话的模型检查默认 `~/.webAgent/config.json`；默认新文件缺失时安全复制旧配置，缺失目标 Key 则用不可跳过的输入门禁阻塞工作区，并记录逐 leaf 模型分配、验证证据与独立审查结论。**卡面写的是桌面宿主，那一态已随 T1 删除**，今天由本机 Node 后端承接同一条通路 |
...
| [core 公开面收敛盘点](core-public-surface-audit.md) | 盘点阶段，无实现：`@einfach-agent/core` 是唯一带 `exports` 通配的包，实测 68 条深导入子路径（非测试 63 / 测试 37）全部构成公开承诺；含五类归类、8 条疑似内部泄漏点名、68→9 的白名单方案、barrel 与 exports 两步走迁移，以及 17 张卡的拆分建议；发包蓝图 G4 的前置 |
