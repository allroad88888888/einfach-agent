...
| [插件 UI Renderer 协议蓝图](plugin-renderer-protocol-blueprint.md) | P2.5 的 R1–R2 已完成（`846743a`、`a2b8d97`）：Core 导出无 React 的时间线 item 投影，React 包提供 root 隔离的 renderer registry 与安全 fallback；R3–R5 待实施 |
...
| [项目路线图](ROADMAP.md) | 本次结构优化范围之外的后续路线：阶段 2/3 的插件产品化与安全发布，再进入阶段 4 的性能与长期运行 |
