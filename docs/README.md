...
| [Node 宿主与 Web 自托管 Issue 树](node-host-issues.md) | 执行中（H 线 12/12 完成，N 线 3/8）：把浏览器版做成能力完整的本地自托管应用——core 的 Tauri invoke 抽成可注入 host bridge，能力实现收敛为一份 `packages/host-node`（约 4700 行 TS），经 `apps/server` 服务浏览器与 CLI，Tauri 最终退成 sidecar 套壳，npm 分发不需要任何签名证书；66 卡分十条线，MVP 路径 50 卡，三项未决待拍板 |
