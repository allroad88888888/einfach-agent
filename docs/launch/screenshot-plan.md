...
给发布物料（README/落地页/商店素材）准备的桌面版界面截图任务清单。所有场景均已对照
实际组件代码核实，未来实现变化时请回来同步本文件里的组件路径与前置状态描述。

截图来源建议用 `pnpm tauri dev` 起桌面 shell 截真机窗口（卡片标题是"桌面版"）；如果暂时没有
桌面构建环境，`pnpm dev` 起的浏览器预览在布局上与桌面端一致，可以先用它出草稿图，正式素材再用
桌面窗口补拍一版。
...
- **主题**：核对过 `apps/web/src/agentNew/ui/agentnew.css`，当前只有一套浅色配色，没有
  ...
  不需要额外出暗色版本。
- **窗口尺寸**：建议桌面窗口不小于 1440×900，保证两栏布局（左侧工作区+会话列表、右侧对话）和弹窗
  内容都不需要横向滚动即可完整入镜。
...
（桌面端才能连），展开已连接服务的工具清单摘要，让"已连接"绿色状态点、工具数量、地址/命令这些信息
都可见。
