...
//   合法 UTF-8 且未被劈开的输入，两边逐字节相同；被劈开时 Node 给的是正确结果。W16/W17 做
//   跨语言对拍时如果撞上这条，该改的是 Rust 侧。
//
// 其余一切（错误文案、上限语义、权限位处理、临时文件命名、`..` 的拒法）都与 Rust 逐条对齐；
// 错误文案刻意保留英文原文，因为它们今天就以这个样子出现在桌面端的工具结果里，两个宿主对同
// 一次越界必须说同一句话。
