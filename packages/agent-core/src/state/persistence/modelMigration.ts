...
export function migrateModelSettings(settings: ModelSettings): ModelSettings {
  const rule = DEPRECATED_MODEL_MIGRATIONS.find(
    (r) => r.vendor === settings.vendor && r.from === settings.model,
  )
  if (!rule) return settings

  const migrated: ModelSettings = { ...settings, model: rule.to }
  if (rule.impliedThinking !== undefined && settings.thinking === undefined) {
    return { ...migrated, thinking: rule.impliedThinking }
  }
  return migrated
}

// 简介：迁移一个 SessionMeta 的 settings；无需迁移时**原样返回同一引用**。
...
export function migrateSessionMeta<T extends { settings: ModelSettings }>(session: T): T {
  const settings = migrateModelSettings(session.settings)
