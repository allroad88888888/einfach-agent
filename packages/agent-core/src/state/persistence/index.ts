...
//
// 收 ./memoryHistoryDriver 的理由（盘点 E6，卡 S7b 判定）：它是 HistoryDriver 契约的
//   参考实现——只 import 本目录的两个类型，零宿主依赖（无 idb / 无 tauri-plugin-sql），
//   语义就是「进程内 Map，不落盘」，不会随 core 内部重构而变。apps/cli 是真实产品消费方
//   （headless 跑一轮不需要盘），让它复制一份实现比公开这一个工厂更糟。
//
