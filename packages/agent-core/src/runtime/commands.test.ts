...
// 【实例化 · 第 2 期】commands 现在经 defaultCore.abort 起停 run（不再从 ./abortRegistry 导入），
//   故 spy 到 defaultCore.abort 的实例方法验证「起/停 run」编排。vite.config 开了 restoreMocks（每个
//   用例前 restoreAllMocks），会把 spyOn 的 spy 还原成真实方法，故必须在 beforeEach 里【每次重建】spy
//   （模块级建一次会被还原掉）。mockImplementation 复刻旧的 abortRegistry mock（beginRun 返回一个 signal，
//   abortRun/endRun 无副作用），避免真登记 AbortController。
let beginRun: MockInstance<typeof defaultCore.abort.beginRun>
let abortRun: MockInstance<typeof defaultCore.abort.abortRun>
let endRun: MockInstance<typeof defaultCore.abort.endRun>
