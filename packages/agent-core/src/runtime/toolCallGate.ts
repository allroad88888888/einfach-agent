...
export function handleToolGate(base: ToolLoopBase, input: ToolGateInput): boolean {
  const gate = selectToolGate({
    ...
    registrationVersion: (name) => base.core.tools.registrationVersion(name),
  ...
  if (gate.kind === 'registration_changed') {
    traceFailure('tool.registration_changed', { registration_changed: true, expectedRegistrationVersion: input.expectedRegistrationVersion, currentRegistrationVersion: base.core.tools.registrationVersion(input.name) }, gate.result)
    return true
  }
  ...
  if (toolName) {
    ...
    const manifest = searchToolManifestPage({ query: typeof input.args.query === 'string' ? input.args.query : undefined, cursor: typeof input.args.cursor === 'string' ? input.args.cursor : undefined, limit: typeof input.args.limit === 'number' ? input.args.limit : undefined }, base.runtimeIsTauri, { registry: base.core.tools })
