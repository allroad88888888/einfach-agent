...
export function createModelTurnRequester(base: ToolLoopBase): ModelTurnRequester {
  ...
  return {
    async request(turn, planStageId, controls) {
      ...
      const contextStats = buildContextStatsSnapshot({ runId: base.runId, turnId: base.turnId, llmTurn: turn + 1, vendor: base.settings.vendor, model: base.settings.model, messages, tools, cacheProfile, cacheTotals, inputBudgetTokens: contextInputBudgetTokens(base.settings.vendor, base.settings.model, base.settings.max_tokens) })
