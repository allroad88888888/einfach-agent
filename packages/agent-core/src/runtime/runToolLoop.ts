...
import { assistantItemFromMessage, tracePreview } from './shared/preview'
...
import { injectStablePrefixTranscript, injectToolTranscript } from './transcriptInjection'
import { createLoopBudget } from './loopBudget'
import {
  accumulateCacheTotals,
  buildContextStatsSnapshot,
  llmTracePreview,
  responseChars,
  toolNames,
  usageStats,
  usageTraceAttrs,
} from './runLoopTelemetry'
import { selectToolGate } from './toolGates'
import {
  closeInterruptedToolCalls,
  currentRunRecoverySnapshot,
  currentTurnItems,
  latestUserInput,
} from './runCheckpoints'
...
export async function runSession(
  ...
  const sessionMeta = core.rootStore.getter(sessionsAtom)[id]
  const traceKey = runTraceKey(id, runId)
  const rootSpan = sessionMeta
    ? startSpan('agent.turn', {
        ...
        attrs: {
          ...
          runId,
          turnId: userItemId,
          vendor: sessionMeta.settings.vendor,
          model: sessionMeta.settings.model,
        },
      })
    : undefined
...
export async function runToolLoop(
  ...
  const originalSessionMeta = core.rootStore.getter(sessionsAtom)[id]
  ...
  const sessionMeta = core.rootStore.getter(sessionsAtom)[id]
  const maxTurnTools = maxTurnToolsForVendor(sessionMeta.settings.vendor)
  ...
  if (sessionMeta !== originalSessionMeta) {
    traceEvent('agent.model_migrated_at_request', {
      from: originalSessionMeta.settings.model,
      to: sessionMeta.settings.model,
    })
  }
  const finishTrace = (
    status: Exclude<TraceStatus, 'running'>,
  ...
  // Stable request items are assembled in modelTurnPrefix so every model turn
  // has one cache-prefix source of truth.
  const stablePrefixState = await buildStableModelPrefix(sessionMeta, core)
  ...
  injectStablePrefixTranscript(id, stablePrefixState, core)
  ...
  const thinking =
    sessionMeta.settings.thinking === undefined
      ? undefined
      : ({ type: sessionMeta.settings.thinking ? 'enabled' : 'disabled' } as const)
  ...
  const sessionTools = Array.isArray(sessionMeta.loadedTools)
    ? sessionMeta.loadedTools.filter((toolName): toolName is string =>
  ...
  const pluginContext = makeCoreCtx({
    sessionId: id,
    runId,
    signal: opts.signal,
    store: core.getSessionStore(id).store,
  ...
  try {
    ...
    if (opts.resumeToolCall) {
      ...
      const resumedToolContext = buildToolContext({
        sessionId: id,
        runId,
        signal: opts.signal,
        callId,
      ...
      try {
        result = await core.tools.run(toolName, args, resumedToolContext, registrationVersion)
    ...
    const turnBudget = createLoopBudget(maxAgentTurns(id, core), () => maxAgentTurns(id, core))
    ...
    for (let turn = 0; turnBudget.allows(turn); turn += 1) {
      ...
      if (promotedAtBoundary > 0) {
        turnBudget.includeQueuedInputs(promotedAtBoundary)
        resetToolFailureStreaks()
      }
      turnBudget.syncPlanFloor()
      ...
      await hooks.transformContext?.(pluginContext, draft)
      if (!isRunningRun(id, runId, core)) {
        commitStoppedTurn()
        finishTrace('cancelled', 'agent.stopped', { reason: 'run_not_running' })
        return
      ...
      const cacheProfile = contextCacheTracker.observe({
        ...
        reasoningEffort: sessionMeta.settings.reasoning_effort,
      ...
      const contextStats = buildContextStatsSnapshot({
        ...
        inputBudgetTokens: contextInputBudgetTokens(
          sessionMeta.settings.vendor,
          sessionMeta.settings.model,
          sessionMeta.settings.max_tokens,
        ),
      })
      ...
      injectToolTranscript(id, tools, toolsFingerprint, core)
      ...
      const requestBase = {
        model: sessionMeta.settings.model,
        messages,
        temperature: sessionMeta.settings.temperature,
        max_tokens: sessionMeta.settings.max_tokens,
        thinking,
        tools,
      ...
      const llmSpan = startSpan('llm.chat', {
          ...
          attrs: () => ({
            ...
            requestPreview: llmTracePreview({
              ...requestBase,
              reasoning_effort: sessionMeta.settings.reasoning_effort,
            }),
          }),
      })
      ...
      const decision = await hooks.onTurnEnd?.(pluginContext, turnEndEvent)
      if (!isRunningRun(id, runId, core)) {
        streamWriter.finishPending()
        commitStoppedTurn()
        finishTrace('cancelled', 'agent.stopped', { reason: 'run_not_running' })
      ...
      if (toolCalls.length > 0) {
          ...
          try {
            const result = await getExecutionRuntime(core).run({
              ...
              task: async (signal) => {
                ...
                return core.tools.run(name, args, toolContext, expectedRegistrationVersion)
              },
            })
            const traced = toolResultTrace(result, args)
            endSpan(toolSpan, traced.status, traced.attrs, traced.err)
        ...
        const parallelCalls = toolCalls.flatMap((toolCall) => {
          ...
          const activeSessionMeta = core.rootStore.getter(sessionsAtom)[id]
        ...
        for (const toolCall of toolCalls) {
          ...
          const toolGate = selectToolGate({
            ...
            loadSchema: (toolName) => {
              ...
              return schema && (schema.runtime !== 'server' || runtimeIsTauri) ? schema : undefined
            },
            expectedRegistrationVersion,
            registrationVersion: (toolName) => core.tools.registrationVersion(toolName),
            canExecuteTool: (toolName) => tools.some((tool) => tool.function.name === toolName),
          ...
          if (toolGate.kind === 'schema_autoloaded') {
            ...
            const autoloadable = toolGate.tool
            visible = ensureToolLoaded(id, visible, name, core, maxTurnTools - 1)
              ...
              const resultPayload = toolGate.result as Record<string, unknown> & { hint?: string }
              const attrs: TraceAttributes = {
                toolName: name,
                callId: toolCall.id,
          ...
          if (toolGate.kind === 'tool_not_allowed') {
          ...
          if (
            toolGate.kind === 'registration_changed'
          ) {
            const resultPayload = toolGate.result
            const error = String(resultPayload.error)
            const attrs: TraceAttributes = {
              toolName: name,
              callId: toolCall.id,
          ...
          if (toolGate.kind === 'schema_request') {
          ...
          const approvalMode = activeSessionMeta?.toolApprovalMode ?? 'confirm'
      ...
      if (promotedAfterReply > 0) {
        turnBudget.includeQueuedInputs(promotedAfterReply)
        resetToolFailureStreaks()
        persistWorkingTurn()
        continue
      }
    ...
    // 代价最大（含用户那条 user 消息）。
    commitTurn()
    const error = `主 Agent 超过最大模型轮次（${turnBudget.limit()}）`
    if (isRunningRun(id, runId, core)) patchRun(id, { status: 'error', error }, core)
    finishTrace('error', 'agent.max_turns', { max_turns: turnBudget.limit(), error })
  } catch (err) {
