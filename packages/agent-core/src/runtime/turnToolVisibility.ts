 ...
 * 闸门改判「有没有桥」之后，**server 宿主**（本地 Node 后端，B 线）会让这些占位变成可见，而
 * stdio 的 Node 实现要等 C 线才有——中间存在「可见但不可用」的窗口。判定为**可接受**：
 *   · 写这段时为零风险：那时唯一有桥的宿主就是桌面端（Tauri，已随 T1 删除），纯浏览器下没有桥，
 *     两种现存宿主的行为与改动前逐字节相同；这个窗口只在一个当时尚不存在的宿主（后来的 server
 *     宿主）上才打开——写这段时它还不叫「非 Tauri 的 server 宿主」，因为 Tauri 尚未退场。
