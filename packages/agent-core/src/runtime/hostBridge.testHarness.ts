...
//
// 【为什么单独一个文件，而不是并进 hostBridgeMock.testHarness.ts】
...
//   · hostBridgeMock（hostBridgeMock.testHarness.ts）管**转换**：整体 mock 掉 hostBridge，喂一个能
 ...
 * 已从一次宿主品牌探测改判为 `hasHostBridge()`，摆布那个全局量对它**再无影响**。
 * 依赖「server 工具对模型可见」的测试若继续摆布那个全局量，用例仍会跑、多数断言仍会过，
 ...
 * 那时 core 自己动态 import 桌面上游模块拿到真 invoke，而 jsdom 里没有宿主内部通道，
