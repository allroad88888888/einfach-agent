...
// invoke 走惰性解析，两者都不再经过上面那个替身。这里把 hostBridge 一并 mock 掉：
...
describe('readWorkspaceRunIndexPage', () => {
  it('映射分页参数并规范化 snake/camel case 响应', async () => {
    ...
    expect(host.invoke).toHaveBeenCalledWith('read_workspace_run_index_page', {
      cursor: 'snapshot:100', max_records: 2, workspace_root: '/workspace',
    })
  })

  it('拒绝缺少 snapshot 的响应，防止不稳定游标进入状态层', async () => {
    host.invoke.mockResolvedValue({ lines: [], hasMore: false })
...
describe('Auto external path bridge inputs', () => {
  it('只读文件桥把 runtime-only 外部路径权限映射为 Tauri snake_case 参数', async () => {
    ...
    expect(host.invoke).toHaveBeenNthCalledWith(1, 'read_workspace_file', expect.objectContaining({
      allow_external_paths: true,
    }))
    expect(host.invoke).toHaveBeenNthCalledWith(2, 'list_workspace_files', expect.objectContaining({
      allow_external_paths: true,
    }))
    expect(host.invoke).toHaveBeenNthCalledWith(3, 'search_workspace_files', expect.objectContaining({
      allow_external_paths: true,
    }))
  })
  ...
  it('rg 桥把 runtime-only 外部路径权限映射为 Tauri snake_case 参数', async () => {
    ...
    expect(host.invoke).toHaveBeenCalledWith('rg_search_workspace', expect.objectContaining({
      allow_external_paths: true,
    }))
  })
})
