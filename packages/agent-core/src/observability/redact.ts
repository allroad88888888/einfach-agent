...
// 多大、压缩有没有生效」时，trace 里只有一排 [REDACTED]，观测性恰好在最需要它的维度上是瞎的。
// （压缩相关的 attr 曾被迫改用 _tk 后缀绕开这条正则，那是绕路；这里把路修直。）
//
// 放行判据【两条必须同时成立】，缺一不可：
//   1) key 以 tokens 结尾 —— 复数意味着计数；密钥惯用单数（access_token / refresh_token /
//      auth_token / token），单数一律继续脱敏。
//   2) 值是有限 number —— 密钥是字符串，计数是数字。
// 第 2 条是保险：即便将来真有人拿 `tokens` 存一组密钥字符串，它照样会被脱敏。
// 收紧或放宽这条之前请先想清楚：这是唯一挡在 API key 落盘之间的东西。
const TOKEN_METRIC_KEY = /tokens$/i

function isTokenMetric(key: string, value: unknown): boolean {
  return TOKEN_METRIC_KEY.test(key) && typeof value === 'number' && Number.isFinite(value)
}
...
function redactValue(
  ...
  try {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (SENSITIVE_KEY.test(key) && !isTokenMetric(key, child)) {
        out[key] = '[REDACTED]'
...
function normalizePreviewValue(
  ...
  try {
    ...
    for (const key of keys.slice(0, options.keyLimit)) {
      ...
      const rawValue = safeProperty(value, key)
      // 与 redactValue 里那处判据保持【完全一致】——两处只要有一处放宽错了，密钥就会落盘。
      out[textKey] = SENSITIVE_KEY.test(textKey) && !isTokenMetric(textKey, rawValue)
        ? '[REDACTED]'
        : normalizePreviewValue(rawValue, depth - 1, seen, options)
    }
