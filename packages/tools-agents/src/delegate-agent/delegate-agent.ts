      ...
      items: {
        ...
        properties: {
          ...
          confirmedTools: {
            type: 'array',
            uniqueItems: true,
            items: { type: 'string', enum: ['shell_macos', 'shell_linux', 'shell_powershell', 'write_file', 'apply_patch'] },
          },
        },
        required: ['objective'],
      },
...
export const delegateAgentTool: Tool = {
  ...
  async execute(args, ctx) {
    ...
    try {
      ...
      const result = await delegateAgents(inputPreservingOptionalPresence(args, normalized))
