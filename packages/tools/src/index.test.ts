...
// core 只提供空注册表 + 抽象；本 meta 包负责把 6 域 21 工具聚合成 registerStandardTools。
...
// 6 域 21 个标准工具的权威清单（顺序 = 注册顺序：shell → interaction → fs → planning → skills → agents）。
...
describe('@web-agent/tools —— 标准工具集聚合（TSPLIT TS2）', () => {
  it('registerStandardTools 恰好装齐 21 个工具', () => {
    const reg = createToolRegistry()
    registerStandardTools(reg)
    expect(reg.list().length).toBe(21)
    expect(STANDARD_TOOLS.length).toBe(21)
  })
