...
同一个工作树里可能同时有**你控制不了的**其它 agent 在改文件、甚至直接提交。默认防着：

- **每批派活前先 `git log --oneline` 看有没有陌生提交**，有就先弄清它改了什么。别人的提交可能已经改掉你子 agent 正在依赖的代码。
- **永远 `git add <具体文件>`，绝不 `git add -A`**。工作树里的改动不都是你的。
- **提交前对 `git status` 逐行认领**：这个文件是哪个 issue 的？不认识就别 stage。
- **子 agent 报「这个测试失败与我无关」时不要直接采信**，也不要用 `git stash` 去验证——那会把别人正在写的文件抽走。安全的查法：确认失败涉及的文件不在改动面内，再用 `git show <可疑commit>^:<file> > <file>` 把**单个文件**临时退回、跑测试、立刻还原。
- **子 agent 必须走公开入口，不许自己重造路径/配置解析。** 别人扩展公开入口时，走入口的代码自动跟着对；自己拼的那份会静默走岔，而且测试抓不到。
