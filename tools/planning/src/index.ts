...
export { createPlanTool, updatePlanTool, executePlanTool, submitStageResultTool }

/** 把 planning 域全部工具注册进给定 registry（幂等：同名覆盖）。 */
export function registerPlanningTools(registry: ToolRegistry): void {
  for (const tool of [createPlanTool, updatePlanTool, executePlanTool, submitStageResultTool]) {
