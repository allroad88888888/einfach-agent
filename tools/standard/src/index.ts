...
// 那 31 个具体工具按能力域拆进 6 个 @einfach-agent/tools-* 包。本 meta 包把它们重新聚合：
//   · re-export 各域的工具对象 + register<Domain>Tools；
//   · 提供 registerStandardTools —— 一把装齐全部 6 域 31 工具，等价于旧的 tools/register.ts。
...
/**
 * 装齐全部 6 域 31 个标准工具（batteries-included）。幂等（各 register 内部同名覆盖）。
