  ...
  return {
    ...
    resolve: {
      ...
      alias: {
        ...
        '@einfach-agent/ai': fromRoot('./packages/agent-ai/src/index.ts'),
        '@einfach-agent/core': fromRoot('./packages/agent-core/src'),
        // Node 宿主能力包。只被 apps/server、apps/cli 与未来的 sidecar 装配层引用，Web 产物里
        // 不该出现——留 alias 是为了 Vitest（root 是仓库根，要解析本包自己的测试）与类型一致。
        '@einfach-agent/host-node': fromRoot('./packages/host-node/src'),
        '@einfach-agent/observability-idb': fromRoot('./packages/observability-idb/src'),
        '@einfach-agent/observability-sqlite': fromRoot('./packages/observability-sqlite/src'),
        '@einfach-agent/persistence-idb': fromRoot('./packages/persistence-idb/src'),
        '@einfach-agent/persistence-sqlite': fromRoot('./packages/persistence-sqlite/src'),
        '@einfach-agent/react-plugin': fromRoot('./packages/agent-react/src/index.ts'),
        '@einfach-agent/subagents': fromRoot('./packages/subagents/src'),
        ...
        '@einfach-agent/tools-agents': fromRoot('./tools/agents/src/index.ts'),
        '@einfach-agent/tools-mcp': fromRoot('./tools/mcp/src/index.ts'),
        '@einfach-agent/tools': fromRoot('./tools/standard/src/index.ts'),
