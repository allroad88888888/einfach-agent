...
async function fixture({ malformed = false } = {}) {
  ...
  const indexRoot = join(base, '.agent-archive', 'index')
  const skillRoot = join(base, '.agent-archive', 'skills')
...
async function governanceState(base) {
  const archiveRoot = join(base, '.agent-archive')
...
describe('subagent skill governance CLI', () => {
  it('is read-only by default and requires --write for mutation', async () => {
    ...
    const before = await readFile(join(base, '.agent-archive', 'skills', 'sk_one.md'), 'utf8')
    ...
    expect(await readFile(join(base, '.agent-archive', 'skills', 'sk_one.md'), 'utf8')).toBe(before)
  })
  ...
  it('promotes one explicit skill and writes index plus audit records', async () => {
    ...
    expect(await readFile(join(base, '.agent-archive', 'skills', 'sk_one.md'), 'utf8')).toContain(
      'promotion: "promoted"',
    )
    const indexLines = (await readFile(join(base, '.agent-archive', 'index', 'skills.jsonl'), 'utf8'))
      .trim()
      .split('\n')
      .map(JSON.parse)
    expect(indexLines.at(-1)).toMatchObject({ skillId: 'sk_one', promotion: 'promoted', governanceAction: 'promote' })
    const audit = JSON.parse(await readFile(join(base, '.agent-archive', 'governance', 'skill-actions.jsonl'), 'utf8'))
  ...
  it('archives a promoted skill as a second explicit audited transition', async () => {
    ...
    const auditLines = (await readFile(join(base, '.agent-archive', 'governance', 'skill-actions.jsonl'), 'utf8'))
      .trim()
      .split('\n')
      .map(JSON.parse)
  ...
  it('fails closed before writing when any index line is malformed', async () => {
    ...
    const path = join(base, '.agent-archive', 'skills', 'sk_one.md')
  ...
  it('fails closed without changing state when the existing audit is malformed', async () => {
    const base = await fixture()
    const governanceRoot = join(base, '.agent-archive', 'governance')
  ...
  it.each([
    ...
    const journalPath = join(base, '.agent-archive', 'governance', 'skill-transaction.json')
  ...
  it('coordinates with the runtime archive-write lock before starting a transaction', async () => {
    const base = await fixture()
    const indexPath = join(base, '.agent-archive', 'index', 'skills.jsonl')
