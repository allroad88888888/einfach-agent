...
fn get_workspace_diff_blocking(
    ...
    let diff_output = run_git_diff_capped(&root, &diff_args(staged, false, &pathspecs), max_diff_chars)?;
...
mod tests {
    ...
    fn diff_args_disable_external_diff_and_textconv() {
        for (staged, stat) in [(false, false), (true, false), (false, true), (true, true)] {
            ...
            assert!(external_idx < diff_idx, "-c diff.external= must precede diff");
