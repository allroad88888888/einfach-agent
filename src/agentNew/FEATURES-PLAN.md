...
| Ta | Ta-1 tauri init | ✅ 已验收（src-tauri 脚手架 + tauri-plugin-sql 接入；cargo check 46.7s 通过） |
| Ta | Ta-2 sqlite driver | ✅ 已验收（sqliteDriver.ts + 5 单测；main.tsx isTauri() 按环境选 driver，sqlite 动态 import 代码分割） |
...
- 2026-07-02：**块 Ta 完成**（Tauri 桌面壳 + SQLite）。`tauri init` 生成 src-tauri（identifier com.webagent.app，窗口 1100×760，devUrl :1420 strictPort）；接入 `tauri-plugin-sql`（Cargo dep + lib.rs 注册 + capabilities `sql:default`/`allow-execute`），`cargo check` 46.7s 编译通过。`state/persistence/sqliteDriver.ts` 用 plugin-sql 实现 HistoryDriver + sessions（建表 + best-effort 降级，5 单测 mock plugin-sql）；`main.tsx` 按 `isTauri()` 选 driver（浏览器 IndexedDB / 桌面 SQLite），sqlite 动态 import 代码分割。web build 干净 + 217 测试全绿。加 `npm run tauri` 脚本。**至此 FEATURES-PLAN 四块（T/D/Ta/C）全部完成。**
