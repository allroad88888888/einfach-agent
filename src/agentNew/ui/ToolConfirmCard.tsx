...
export function ToolConfirmCard() {
  ...
  const { callId, toolName, args } = run.pendingToolConfirmation
  ...
  return <ConfirmCardBody key={callId} toolName={toolName} args={args} />
}

// 单次确认的卡片主体：勾选/按钮等本地态都在这里，靠父组件 key={callId} 保证每次确认复位。
function ConfirmCardBody({ toolName, args }: { toolName: string; args: unknown }) {
  ...
  return (
    <section className="agentnew-confirm" aria-labelledby="agentnew-confirm-title">
      <header className="agentnew-confirm-header">
        <span className="agentnew-confirm-eyebrow">需要确认</span>
      ...
      <label className="agentnew-confirm-always">
        <input
          type="checkbox"
          checked={always}
      ...
      <footer className="agentnew-confirm-footer">
        ...
        <button
          ...
          onClick={() => confirmTool(true, always)}
        >
          允许
        </button>
      </footer>
