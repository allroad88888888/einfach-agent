// apps/web/src/plugins/workspacePluginProvider.ts —— 浏览器宿主的 PluginSettingsProvider 实现
// ---------------------------------------------------------------------------
...
//
// 【T1：文件名与导出名从 desktopProvider / createDesktopPluginSettingsProvider 改过来】
// 它从来就不依赖桌面端任何专属能力——文件系统桥复用项目 skills 那条通路
// （buildProjectSkillsWorkspaceBridge），而那个判据早在 H 线换成了 `hasHostBridge()`。桌面端
// 退出后它是**唯一**的插件 provider，仍叫 desktop 会让下一个照着「删掉 desktop* 模块」清单
// 干活的人把活的实现删掉。
//
// pluginScanner.ts 的 PluginScanBridge 就是照着 ProjectSkillsLoaderBridge 的形状定的，
// 结构上完全兼容：宿主侧不需要为插件扫描新增任何 command，`.webAgent/plugins/` 本就在
...
import { createPluginImportModule } from './pluginImportModule'
...
export interface WorkspacePluginSettingsProviderOptions {
  /**
   * 扫描哪个 workspace。缺席 = 当前会话还没有 workspace root（页面刚起、workspace 还没 hydrate
   * 回来），此时 load() 返回空清单：宿主仍然支持插件，只是没有可扫的目录，不该谎报「不支持」。
   */
  workspaceRoot?: string
  /** 覆盖文件系统桥（测试注入内存 fake；生产走项目 skills 的同一条宿主命令桥通路）。 */
  bridge?: PluginScanBridge
  /** 覆盖模块求值（测试注入；生产走 blob URL 求值，见 pluginImportModule.ts）。 */
...
export function createWorkspacePluginSettingsProvider(
  options: WorkspacePluginSettingsProviderOptions = {},
): PluginSettingsProvider {
  const { workspaceRoot } = options

  // 桥创建推迟到首次扫描；桥内部的宿主调用仍在 hasHostBridge() 守卫后惰性加载。
  ...
  const resolveBridge = async (): Promise<PluginScanBridge> => {
    ...
    // 只在**没有宿主命令桥**时发生——判据是 `hasHostBridge()`，不是「是不是某种宿主」
    // （B8 复核：H4c 之后 buildProjectSkillsWorkspaceBridge 早就换了判据，这段注释此前还停在旧机制）。
    // 装配层不会在 static 宿主里造本 provider，走到这里说明装配被绕过了，如实报错。
  ...
  const loaderDeps = (root: string, isToolEnabled: PluginToolGate): PluginLoaderDeps => ({
    importModule: options.importModule ?? createPluginImportModule(root),
  ...
  return {
    // 造出本 provider 的宿主恒支持插件：有没有可扫的目录是 workspace 的事实，不是宿主能力的事实。
