// apps/web/src/plugins/pluginImportModule.ts —— 浏览器宿主的插件入口求值
// ---------------------------------------------------------------------------
// P4 的 loader 只声明「给我一个把 entryPath 变成模块的函数」（pluginLoaderTypes.ts 的
// importModule），怎么变是宿主的事。浏览器这条按蓝图 3.4 走：
// 经宿主命令桥读文件 → 前端 blob URL → 动态 import 求值。
//
// 【CSP】apps/web/index.html 没有 <meta http-equiv="Content-Security-Policy">，`apps/server`
// 也不注入 CSP 头，因此 blob: 脚本求值不受策略拦截。将来若加 CSP，必须同时放行
// script-src blob:，否则本文件这条路径会被静默挡死。
//
// 【这是在渲染进程里执行第三方代码】与页面同权：可触达宿主命令桥、DOM、fetch。
 ...
 */
export function createPluginImportModule(
