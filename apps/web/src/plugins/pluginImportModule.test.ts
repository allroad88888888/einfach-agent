...
import { createPluginImportModule, PLUGIN_ENTRY_READ_LIMIT } from './pluginImportModule'

const { readWorkspaceFileMock } = vi.hoisted(() => ({ readWorkspaceFileMock: vi.fn() }))

// 局部 mock：契约模块桥会静态 import `@einfach-agent/core/plugin`（进而把整个 runtime 拉进
...
describe('createPluginImportModule', () => {
  beforeEach(() => {
  ...
  it('把入口读进来求值成模块，并在求值后回收 blob URL', async () => {
    ...
    const importModule = createPluginImportModule(WORKSPACE_ROOT, { evaluate })
  ...
  it('求值抛错：blob URL 照样回收，异常原样上抛给 loader', async () => {
    ...
    const importModule = createPluginImportModule(WORKSPACE_ROOT, {
      evaluate: async () => {
