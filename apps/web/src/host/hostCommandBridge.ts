// 把当前宿主的命令桥登记进 core —— 两态里只有 `server` 有桥，`static` 什么都不登记。
// ---------------------------------------------------------------------------
...
//   · 先于 initializePluginSettings()——workspacePluginProvider 的 resolveBridge() 会求值一次
...
//
// 【为什么 loader 由装配层自己持有】core 一侧不提供任何具体宿主的 loader（那种东西不在
// `@einfach-agent/core` 的公开面上，深导入 `runtime/host*` 子路径会撞 check-boundaries 的 core
// 公开面白名单，S9）。这也正是 H 线的方向：桥背后是什么由宿主说了算，core 不必认识任何一种宿主。
//
...
// HostBridgeRegistration）。**平台不能靠本地探测**：浏览器跑在用户机器上（macOS），执行命令的是
// 服务端那台（可能是 Linux），`navigator.userAgent` 答的是前者、而校验它的是后者。平台只能取自
// `GET /api/health` 的握手（resolveHost 已把它带在 `kind: 'server'` 这一支上，类型上漏不掉）。
// 握手报回 `'unsupported'`（FreeBSD / AIX 这类）时原样传下去：core 认识第四个值，
// 在这里替它映射成三选一等于谎报，而后果是每条 shell 命令都撞 platform mismatch。
//
// 【T1 删掉了什么】这里曾有一支 `case 'tauri'`：`() => Promise.resolve(invoke)` + 本地探测平台
// （桌面 webview 与原生同机，那条前提当时成立）。桌面端整条退出后它连同上游包一起删了。
 ...
 * 按解析出的宿主登记命令桥。
 *
 * loader 的类型是 `() => Promise<HostInvoke>`——得包一层函数，直接把 invoke 本体传进去编译不过。
 */
