...
export function PlanPanel() {
  ...
  const planStatusText = {
    draft: t`草稿`, awaiting_approval: t`待批准`, approved: t`已批准`, active: t`执行中`,
    completed: t`已完成`, failed: t`未通过`, cancelled: t`已取消`,
  }
  if (!plan) {
    ...
    return (
      <section className="agentnew-plan is-drafting" aria-labelledby="agentnew-plan-title">
        <header className="agentnew-plan-header">
          <div>
            <span className="agentnew-plan-eyebrow">{t`正在制定计划`}</span>
            <h2 id="agentnew-plan-title" className="agentnew-plan-title">{t`需要你的决策`}</h2>
            <p className="agentnew-plan-objective">{t`回答后会从当前位置继续生成同一份计划。`}</p>
          </div>
          <span className="agentnew-plan-status is-awaiting-decision">{t`等待决策`}</span>
        </header>
  ...
  return (
    <section className="agentnew-plan" aria-labelledby="agentnew-plan-title">
      <header className="agentnew-plan-header">
        <div>
          <span className="agentnew-plan-eyebrow">{t`执行计划 · r${plan.revision}`}</span>
        ...
        <div className="agentnew-plan-header-actions">
          <span className={`agentnew-plan-status ${planDecision ? 'is-awaiting-decision' : canContinue ? 'is-paused' : `is-${plan.status}`}`}>
            {planDecision ? t`等待决策` : canContinue ? t`待继续` : planStatusText[plan.status]}
          </span>
          <button
            type="button"
            ...
            aria-label={planPanelExpanded ? t`收起计划详情` : t`展开计划详情`}
            onClick={() => setPlanPanelExpanded((current) => !current)}
          >
            {planPanelExpanded ? t`收起` : t`展开`}
          </button>
        </div>
      </header>
        ...
        {plan.stages.map((stage) => {
            ...
            <div className="agentnew-plan-stage-card">
              ...
              >
                ...
                <span className="agentnew-plan-stage-status-actions">
                  <button
                    ...
                    title={stage.status === 'pending'
                      ...
                      : stageRollbackPoints.has(stage.id)
                        ? t`回到该阶段开始前：恢复当时的计划快照，并撤回该阶段之后的对话`
                        : t`回滚该阶段及其后续依赖阶段（该阶段没有回退点，对话不会被撤回）`}
                    onClick={(event) => {
                  ...
                  <span>{awaitingDecision ? t`等待决策` : pausedStage ? t`待继续` : statusText[stage.status]}</span>
                </span>
              ...
              {expanded ? <div className="agentnew-plan-stage-body">
                  ...
                  <div>
                    ...
                    <span>
                      {stage.dependencies.length > 0
                        ? stage.dependencies.map((id) => stageTitleById.get(id) ?? id).join(t`、`)
                        : t`无`}
                    </span>
                  </div>
                </div>
                ...
                {stage.evidence.length > 0 && (
                  <div className="agentnew-plan-evidence">{t`证据：${stage.evidence.join(t`；`)}`}</div>
                )}
                {stage.blockReason && <div className="agentnew-plan-block">{t`阻塞：${stage.blockReason}`}</div>}
          ...
          {awaitingApproval && (
            <footer className="agentnew-plan-approval">
              <span>{t`请确认这份计划后再开始执行。`}</span>
              <div>
                <button type="button" className="agentnew-confirm-reject" onClick={() => void containPlanCommand(approvePlan(false))}>{t`拒绝`}</button>
                <button type="button" className="agentnew-confirm-approve" onClick={() => void containPlanCommand(approvePlan(true))}>{t`批准并继续`}</button>
              </div>
            </footer>
          )}
          {canContinue && (
            <footer className="agentnew-plan-approval agentnew-plan-resume">
              <span>{t`这是上次保存的计划进度，当前没有 Agent 在运行。`}</span>
              <button type="button" className="agentnew-confirm-approve" onClick={continuePlan}>{t`继续执行`}</button>
            </footer>
          )}
        </div>
      )}
